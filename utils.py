...
def get_provider_data(provider_name, world_gdf, rows=1000):
    ...
    gdf = gpd.GeoDataFrame(
        ...
        geometry=gpd.points_from_xy(
            myquery_df.edmPlaceLongitude,
            myquery_df.edmPlaceLatitude),
        crs="EPSG:4326"
    )

    # Perform spatial join
    gdf_with_country = gpd.sjoin(
        gdf, world_gdf[['geometry', 'name']], how='left', op='within')

    # Add the new column to the original DataFrame
    myquery_df['object_location'] = gdf_with_country['name']

    # Fill NaN values (points that don't fall within any country) with
    # "Unknown"
    myquery_df['object_location'] = myquery_df['object_location'].fillna(
        "Unknown")
    ...
    world_counts['object_count'] = world_counts['name'].map(
        country_counts).fillna(0).astype(int)
